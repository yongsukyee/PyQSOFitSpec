...
# Adapted from PyQSOFit and PyQSOFit_SBL
# Original PyQSOFit code: https://github.com/legolason/PyQSOFit/blob/master/PyQSOFit.py, 11 Feb 2022, version 1.1
#   Author: Hengxiao Guo AT UCI
#   Email: hengxiaoguo AT gmail DOT com
#   Co-Author Shu Wang, Yue Shen, Wenke Ren
# Modified PyQSOFit_SBL code: https://github.com/JackHon55/PyQSOFit_SBL, 8 May 2022
#   Modified by: Jack Hon (Wei Jeat)
# Last modified on 16 Jun 2022
# Modified by: Suk Yee Yong, 16 Jun 2022
# -------------------------------------------------
...
def get_color(color):
    """Get color blind friendly pallete if available"""
    assert color in ['orange', 'cyan', 'green', 'olive', 'blue', 'red', 'pink'], f"color={color} not available. Choose from ['orange', 'cyan', 'green', 'olive', 'blue', 'red', 'pink']."
    try:
        from cb_palette import cb_color
        return cb_color(color)
    except ImportError:
        print('cb_palette.py not found. Using default matplotlib tab color')
        return 'tab:'+color
...
class QSOFit():
    ...
    def Fit(self, name=None, nsmooth=1, and_or_mask=True, reject_badpix=True, deredden=True, wave_range=None,
            ...
            linemodel='skewnorm_model', linefit=True, save_result=True, plot_fig=True, save_fig=True, plot_line_name=True,
            plot_legend=True, dustmap_path=None, save_fig_path=None, save_fits_path=None, save_fits_name=None):
        ...
        linemodel: str of function defined in fit_linemodel.py, optional
            model to fit line. 'gauss_model' uses Gaussian. Default: 'skewnorm_model' uses skew normal
        ...
        save_fits_name: str, optional
            the output name of the result fits. If True, "result.fits". If None, the default is the result fits is not saved
        ...
        self.linemodel = getattr(fit_linemodel, linemodel)
        ...
        if save_fits_name:
            if isinstance(save_fits_name, str):
    ...
    def _RestFrame(self, lam, flux, err, z):
        """Move wavelength and flux to rest frame"""
        self.wave = lam/(1. + z)
        self.flux = flux*(1. + z)
        self.err = err*(1. + z)
    ...
    def _OrignialSpec(self, wave, flux, err):
        """save the original spectrum before host galaxy decompsition"""
    ...
    def _DoContiFit(self, wave, flux, err, ra, dec, plateid, mjd, fiberid):
        ...
        global fe_uv, fe_op
        fe_uv = np.genfromtxt(os.path.join(self.path, 'fe_uv.txt'))
        fe_op = np.genfromtxt(os.path.join(self.path, 'fe_optical.txt'))
        ...
        if self.MC and self.n_trails > 0:
            ...
            self.conti_result = np.array(
                [ra, dec, plateid, mjd, fiberid, self.z, self.SN_ratio_conti, conti_fit.params[0],
    ...
    def _DoLineFit(self, wave, line_flux, err, f):
        ...
        # read line parameter
        linepara = fits.open(os.path.join(self.path, 'qsopar2.fits'))
        ...
        if ind_kind_line.any():
            ...
            for ii in range(ncomp):
                ...
                if np.sum(ind_n) > 10:
                    ...
                    # plt.plot(np.log(wave[ind_n]), self.MultiLine_Model(self.linemodel)(np.log(wave[ind_n]), line_fit.params))
                    # calculate MC err
                    if self.MC and self.n_trails > 0:
                        all_para_std, fwhm_std, sigma_std, skew_std, ew_std, peak_std, area_std = self._line_mc(
                            np.log(wave[ind_n]), line_flux[ind_n], err[ind_n], self.line_fit_ini, self.line_fit_par,
                            self.n_trails, compcenter, ind_line)
                    ...
                    for n in range(nline_fit):
                        for nn in range(int(ngauss_fit[n])):
                            ...
                            if self.MC and self.n_trails > 0:
                                gauss_type_tmp_tmp = ['float', 'float', 'float', 'float', 'float', 'float', 'float', 'float']
                                gauss_name_tmp_tmp = [line_name + '_scale', line_name + '_scale_err',
                                                      ...
                                                      line_name + '_skewness_err']
                            else:
                                gauss_type_tmp_tmp = ['float', 'float', 'float', 'float']
                                gauss_name_tmp_tmp = [line_name + '_scale', line_name + '_centerwave',
                                                      line_name + '_sigma', line_name + '_skewness']
                    ...
                    if self.MC and self.n_trails > 0:
                        fur_result_tmp = np.array(
                            [fwhm, fwhm_std, sigma, sigma_std, skew, skew_std, ew, ew_std, peak, peak_std, area, area_std])
                        ...
                        fur_result_name_tmp = np.array(
                            [br_name+'_whole_br_fwhm', br_name+'_whole_br_fwhm_err', br_name+'_whole_br_sigma',
                             br_name+'_whole_br_sigma_err', br_name+'_whole_br_skew', br_name+'_whole_br_skew_err',
                             br_name+'_whole_br_ew', br_name+'_whole_br_ew_err', br_name+'_whole_br_peak',
                             br_name+'_whole_br_peak_err', br_name+'_whole_br_area', br_name+'_whole_br_area_err'])
                    else:
                        fur_result_tmp = np.array([fwhm, sigma, skew, ew, peak, area])
                        fur_result_type_tmp = np.concatenate(
                            [fur_result_type_tmp, ['float', 'float', 'float', 'float', 'float', 'float']])
                        fur_result_name_tmp = np.array(
                            [br_name+'_whole_br_fwhm', br_name+'_whole_br_sigma', br_name+'_whole_br_ew',
                             br_name+'_whole_br_skew', br_name+'_whole_br_peak', br_name+'_whole_br_area'])
    ...
    def _do_line_kmpfit(self, linelist, line_flux, ind_line, ind_n, nline_fit, ngauss_fit):
        ...
        for n in range(nline_fit):
            for nn in range(ngauss_fit[n]):
                ...
                line_fit_ini0 = [ini_flx, np.log(linelist['lambda'][ind_line][n]), ini_sig, ini_skw]
    ...
    def _line_mc(self, x, y, err, pp0, pp_limits, n_trails, compcenter, ind_line):
        ...
        for tra in range(n_trails):
            ...
            all_fwhm[tra], all_sigma[tra], all_skew[tra], all_ew[tra], all_peak[tra], all_area[tra] \
                = self.line_prop(compcenter, line_fit.params, 'broad')
        
        for st in range(len(pp0)):
            all_para_std[st] = all_para_1comp[st, :].std()
        
        return all_para_std, all_fwhm.std(), all_sigma.std(), all_skew.std(), all_ew.std(), all_peak.std(), all_area.std()
    ...
    def line_prop(self, compcenter, pp, linetype):
        ...
        ind_br[16:] = False  # to exclude the broad OIII and broad He II
        ...
        if n_gauss == 0:
            ...
            yy = self.MultiLine_Model(self.linemodel)(xx, pp)
            ...
            if n_gauss > 3:
                if np.max(self.MultiLine_Model(self.linemodel)(xx, pp[0:16])) > 0:
                    spline = interpolate.UnivariateSpline(xx, self.MultiLine_Model(self.linemodel)(xx, pp[0:16]) -
                                                          np.max(self.MultiLine_Model(self.linemodel)(xx, pp[0:16])) / 2, s=0)
                else:
                    spline = interpolate.UnivariateSpline(xx, self.MultiLine_Model(self.linemodel)(xx, pp[0:16]) -
                                                          np.min(self.MultiLine_Model(self.linemodel)(xx, pp[0:16])) / 2, s=0)
            else:
                if np.max(yy) > 0:
            ...
            if len(spline.roots()) > 0:
                ...
                line_flux = self.MultiLine_Model(self.linemodel)(xx, pp)
                line_wave = np.exp(xx)
    ...
    def _residuals_line(self, pp, data):
        ...
        # restore parameters
        self.newpp = pp.copy()
        return (yval - self.MultiLine_Model(self.linemodel)(xval, pp)) / weight
    ...
    def _SaveResult(self, conti_result, conti_result_type, conti_result_name, line_result, line_result_type,
        ...
        all_result_type = np.concatenate([conti_result_type, line_result_type])
        all_result_name = np.concatenate([conti_result_name, line_result_name])
        
        t = Table(all_result, names=(all_result_name), dtype=all_result_type)
        if save_fits_name:
            t.write(os.path.join(save_fits_path, f"spec-{save_fits_name}_fit.fits"), format='fits', overwrite=True)
    ...
    def _PlotFig(self, ra, dec, z, wave, flux, err, decomposition_host, linefit, tmp_all, gauss_result, f_conti_model,
        ...
        if linefit:
            ...
            # Configure lines for plot
            line_kwargs = {
                'linename': {'color': 'grey', 'ls': '--', 'alpha': 0.7},
                'data': {'color': 'k', 'lw': '0.7', 'alpha': 0.4, 'label': 'data'},
                'fit': {'color': get_color('orange'), 'ls': '-.', 'lw': '2', 'alpha': 0.8, 'label': 'fit', 'zorder': 4},
                'fitbr': {'color': get_color('blue'), 'lw': '2', 'alpha': 0.8, 'label': 'fit broad', 'zorder': 3},
                'fitna': {'color': get_color('cyan'), 'lw': '2', 'alpha': 0.8, 'label': 'fit narrow', 'zorder': 6},
                'continuum': {'color': get_color('olive'), 'lw': '2', 'alpha': 0.8, 'label': 'continuum', 'zorder': 9},
                'feii': {'color': get_color('green'), 'lw': '2', 'alpha': 0.8, 'label': 'FeII', 'zorder': 7},
                # Have not configure decomposition_host, BC
            }
            
            temp_gauss_result = gauss_result
            for p in range(int(len(temp_gauss_result)/mc_flag/4)):
                # warn that the width used to separate narrow from broad is not exact 1200 km s-1 which would lead to wrong judgement
                # Narrow line
                if self.CalFWHM(temp_gauss_result[(2+p*4)*mc_flag]) < 1200.:
                    line_kws = line_kwargs['fitna']
                # Broad line
                else:
                    line_kws = line_kwargs['fitbr']
                
                line_single = self.linemodel(np.log(wave), temp_gauss_result[p*4*mc_flag:(p+1)*4*mc_flag:mc_flag])
                
                ax.plot(wave, line_single+f_conti_model, **line_kws)
                for c in range(self.ncomp):
                    axn[1][c].plot(wave, line_single, **line_kws)
                lines_total += line_single
            
            ax.plot(wave, lines_total+f_conti_model, **line_kwargs['fit'])  # supplement the emission lines in the firs subplot
            for c in range(self.ncomp):
                axn[1][c].plot(wave, lines_total, **line_kwargs['fit'])
                axn[1][c].plot(wave, self.line_flux, zorder=0, **line_kwargs['data'])
                ...
                axn[1][c].text(0.02, 0.9, uniq_linecomp_sort[c], fontsize=18, transform=axn[1][c].transAxes)
        ...
        if self.ra == -999. or self.dec == -999.:
            ax.set_title('spec-'+str(self.sdss_name)+'   z = '+str(np.round(z, 4)), fontsize=20)
        else:
            ax.set_title('ra,dec = ('+str(ra)+','+str(dec)+')   '+str(self.sdss_name)+'   z = '+str(np.round(z, 4)), fontsize=20)
        
        ax.plot(self.wave_prereduced, self.flux_prereduced, zorder=2, **line_kwargs['data'])
        ...
        # ax.scatter(wave[tmp_all], np.repeat(self.flux_prereduced.max()*1.05, len(wave[tmp_all])), color='grey', marker='o')# plot continuum region
        
        ax.plot(wave, f_conti_model, **line_kwargs['feii'])
        if self.BC:
            ax.plot(wave, self.f_pl_model+self.f_poly_model+self.f_bc_model, 'y', lw=2, label='BC', zorder=8)
        ax.plot(wave, conti_fit.params[6]*(wave/3000.0)**conti_fit.params[7]+self.F_poly_conti(wave, conti_fit.params[11:]), **line_kwargs['continuum'])
        ...
        if self.plot_legend:
            ...
            ax.legend(by_label.values(), by_label.keys(), loc='best', frameon=False, ncol=2, fontsize=10)
        
        # plot line name--------
        if self.plot_line_name:
            line_cen = np.array(
                [6564.60, 6549.85, 6585.27, 6718.29, 6732.66, 4862.68, 5008.24, 4687.02, 4341.68, 3934.78, 3728.47,
                 3426.84, 2798.75, 1908.72, 1857.40, 1816.97, 1750.26, 1718.55, 1549.06, 1640.42, 1402.06, 1396.76, 1335.30, \
                 1215.67])
            
            line_name = np.array(
                ['', '', 'Ha+[NII]', '', '[SII]6718,6732', 'Hb', '[OIII]', 'HeII4687', 'Hr', 'CaII3934', '[OII]3728',
                 'NeV3426', 'MgII', 'CIII]', 'AlIII', 'SiII1816', 'NIII]1750', 'NIV]1718', 'CIV', 'HeII1640', '', 'SiIV+OIV',
                 'CII1335', 'Lya'])
            
            for ll in range(len(line_cen)):
                if wave.min() < line_cen[ll] < wave.max():
                    ax.plot([line_cen[ll], line_cen[ll]], [plot_bottom*0.9, self.flux_prereduced.max()*1.1], **line_kwargs['linename'])
                    ax.text(line_cen[ll]+7, 1.08*self.flux_prereduced.max(), line_name[ll], rotation=90, fontsize=10, va='top')
        
        ax.set_xlim(wave.min(), wave.max())
        # Change label size for both axes
        ax.tick_params(axis='both', which='major', labelsize=16)
        for a in axn[1]:
            a.tick_params(axis='both', which='major', labelsize=16)
        
        if linefit:
            ax.text(0.5, -1.45, r'$\rm Rest \, Wavelength$ ($\rm \AA$)', fontsize=20, transform=ax.transAxes,
                    ha='center')
            ax.text(-0.07, -0.1, r'$\rm f_{\lambda}$ ($\rm 10^{-17} erg\;s^{-1}\;cm^{-2}\;\AA^{-1}$)', fontsize=20,
        ...
        if self.save_fig:
            plt.savefig(os.path.join(save_fig_path, f"spec-{self.sdss_name}{'_fit' if linefit else ''}.pdf"), bbox_inches='tight')
    ...
    def MultiLine_Model(self, linemodel):
        """The multi linemodel used to fit the emission lines, it will call the self.linemodel function"""
        def fit_multilinemodel(xval, pp):
            ngauss = int(pp.shape[0]/4)
            if ngauss != 0:
                yval = 0.
                for i in range(ngauss):
                    yval = yval + self.linemodel(xval, pp[i*4:(i+1)*4])
                return yval
            else:
                return np.zeros_like(xval)
        return fit_multilinemodel
